...
public class Network implements Serializable{
    ...
    private List<MyDenseDoubleMatrix2D> weightMatrixList = null;
    private List<MyDenseDoubleMatrix2D> biasMatrixList = null;
    private boolean train = false;

    public List<MyDenseDoubleMatrix2D> getWeightMatrixList() {
		return weightMatrixList;
	}

	public void setWeightMatrixList(List<MyDenseDoubleMatrix2D> weightMatrixList) {
		this.weightMatrixList = weightMatrixList;
	}

	public List<MyDenseDoubleMatrix2D> getBiasMatrixList() {
		return biasMatrixList;
	}

	public void setBiasMatrixList(List<MyDenseDoubleMatrix2D> biasMatrixList) {
		this.biasMatrixList = biasMatrixList;
	}
    ...
    public void initWeightAndBais(){
        ...
        this.biasMatrixList = new ArrayList<>();

        //initialize weight and bias
        for(int i=0;i<this.layerNum-1;i++){
            int colNumber = this.sizes[i];
            int rowNumber = this.sizes[i+1];
            MyDenseDoubleMatrix2D weightMatrix = new MyDenseDoubleMatrix2D(rowNumber,colNumber);
        ...
        for(int i=1;i<this.layerNum;i++){
            ...
            MyDenseDoubleMatrix2D biasMatrix = new MyDenseDoubleMatrix2D(rowNumber,colNumber);
            double[][] randomMatrix = new double[rowNumber][colNumber];
        ...
        for(int i=0;i<iterNums;i++){
            for(int j=0;j<trainData.size();j++){
                ...
                for(int m=layerNum-2;m>=0;m--){
                    ...
                    MyDenseDoubleMatrix2D copyWeight = new MyDenseDoubleMatrix2D(weightMatrixList.get(m).toArray().length,
                            weightMatrixList.get(m).toArray()[0].length);
                    copyWeight.assign(weightMatrixList.get(m));
                    weightMatrixList.set(m,(MyDenseDoubleMatrix2D)copyWeight.assign(deltaWeight,plus));

                    MyDenseDoubleMatrix2D copyErrorMatrix = new MyDenseDoubleMatrix2D(errMatrix[m+1].toArray().length,
                            errMatrix[m+1].toArray()[0].length);
                    copyErrorMatrix.assign(errMatrix[m+1]);
                    MyDenseDoubleMatrix2D deltaBias = (MyDenseDoubleMatrix2D)copyErrorMatrix.assign(learnRatioFunction1);
                    MyDenseDoubleMatrix2D copyBiasMatrix = new MyDenseDoubleMatrix2D(biasMatrixList.get(m).toArray().length,
                            biasMatrixList.get(m).toArray()[0].length);
                    copyBiasMatrix.assign(biasMatrixList.get(m));
                    biasMatrixList.set(m, (MyDenseDoubleMatrix2D) copyBiasMatrix.assign(deltaBias, plus));
                }
            }
